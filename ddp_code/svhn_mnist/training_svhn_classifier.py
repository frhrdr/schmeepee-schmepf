...
def cifar10_loader(batch_size):
    ...
    test_vi = torchvision.datasets.CIFAR10(root='./data', transform=transforms.ToTensor(),download=True)
    test_vii = torch.utils.data.DataLoader(test_vi, batch_size=batch_size, shuffle=True)
...
def fit_one_cycle(epochs, max_lr, model, train_loader, val_loader,
    ...
    torch.save({'epoch': epoch,
            ...
            'loss': loss}, 'Trained_ResNet_cifar10') # previously, Trained_ResNet
...
def main():
    ...
    test_vii, train_dl, test_dl = cifar10_loader(batch_size)
    ...
    # Resnet_model = to_device(ResNet(in_channels=3, num_classes=num_classes, get_perceptual_feats=False), device)
    Resnet_model = resnet_18(get_perceptual_feats=True, num_classes=num_classes, image_size=28).to(device)
    print(Resnet_model)
