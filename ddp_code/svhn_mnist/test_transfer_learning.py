...
def load_trained_model(model2load, model, domain_adapt, which_layer_to_finetune, device):
    ...
    if model=='VGG':
        ...
        net = resnet_18(get_perceptual_feats=True, num_classes=10, image_size=28).to(device)
        checkpoint = torch.load(model2load)
    ...
    if domain_adapt:
        ...
        for param in net.parameters():
            ...
            if len(which_layer_to_finetune)==0:
                # print(param.shape)
                param.requires_grad = False
            elif len(which_layer_to_finetune)==1:
                if count == 60:
                    param.requires_grad = True
                else:
                    param.requires_grad = False
            elif len(which_layer_to_finetune)==2:
                if count == 60 or count == 0:
                    param.requires_grad = True
                else:
                    param.requires_grad = False
            elif len(which_layer_to_finetune)==3:
                if count == 60 or count == 0 or count==1:
                    param.requires_grad = True
                else:
                    param.requires_grad = False
            else:
                print('we only finetune the first three layers')
...
def data_loader(batch_size, model, data):
    ...
    if data == 'digits':
        ...
        train_dataset = datasets.MNIST(root='data', train=False, transform=transform_train, download=True)
        ...
        train_dataset = torchvision.datasets.SVHN(root, split='train', transform=transform, target_transform=None,
                                          download=True)
        test_dataset = torchvision.datasets.SVHN(root, split='test', transform=transforms.ToTensor(), download=True)
