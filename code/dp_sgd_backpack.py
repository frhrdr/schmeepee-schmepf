...
def dp_sgd_backward_debug(params, loss, device, clip_norm, noise_factor, debug=True, terminate_after_iteration=True):
  ...
  for idx, param in enumerate(params):
    ...
    noise_sqrd_sum_acc += pt.sum(dp_noise ** 2)
    grad_sqrd_sum_acc += pt.sum(clipped_grad ** 2)
  ...
  if debug:
    noise_norm = pt.sqrt(noise_sqrd_sum_acc)
    grad_norm = pt.sqrt(grad_sqrd_sum_acc)
    print(f'noise norm: {noise_norm}, grad_norm: {grad_norm}, SNR: {grad_norm / noise_norm}')
